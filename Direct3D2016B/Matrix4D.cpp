...
MATRIX4D RotationAxis(float theta, VECTOR4D& A)
{
	/*
	theta = -theta;
	float c = cosf(theta);
	...
	R.m00 = c + _1c * A.x * A.x;
	R.m10 = _1c*(A.x*A.y) - s*A.z;
	R.m20 = _1c*(A.z*A.x) + s*A.y;

	R.m01 = _1c*(A.x*A.y) + s*A.z;
	R.m11 = c + _1c*(A.y*A.y);
	R.m21 = _1c * (A.z*A.y) - s*A.x;

	R.m02 = _1c*(A.z*A.x) - s*A.y;
	R.m12 = _1c*(A.z*A.y) + s*A.x;
	R.m22 = c + _1c *A.z*A.z;
